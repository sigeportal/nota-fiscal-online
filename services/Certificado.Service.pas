...
type
	...
	public
		class function UploadCertificado(const ABase64: string; const ASenha: string; const ACNPJ: string; const AUsuarioId: Integer): TCertificadoResult;
	...
	private
		...
		class function ExtrairInfoCertificado(const ADados: TBytes; const ASenha: string; out AAssunto: string; out AEmissor: string; out AValIni: TDateTime; out AValFim: TDateTime; out AThumb: string): Boolean;
	end;
...
class function TCertificadoService.ExtrairInfoCertificado(const ADados: TBytes; const ASenha: string; out AAssunto: string; out AEmissor: string; out AValIni: TDateTime; out AValFim: TDateTime; out AThumb: string): Boolean;
var
	LACBrNFe: TACBrNFe;
	LTmpFile: string;
begin
	...
	try
		...
		LACBrNFe.SSL.ArquivoPFX := LTmpFile;
		LACBrNFe.SSL.Senha      := ASenha;
...
class function TCertificadoService.UploadCertificado(const ABase64: string; const ASenha: string; const ACNPJ: string; const AUsuarioId: Integer): TCertificadoResult;
var
	LDados    : TBytes;
	LAssunto  : string;
	LEmissor  : string;
...
begin
	...
	TLogger.Info('Certificado.Service: Upload OK CNPJ=%s codigo=%d validade=%s', [LCNPJNum, LCodigo, DateTimeToStr(LValFim)]);
end;
