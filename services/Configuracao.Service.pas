...
type
	...
	public
		...
		class function Salvar(const AUsuarioId: Integer; const ABody: TJSONObject): TConfiguracaoResult;
	end;
...
begin
	...
	try
		...
		try
			LCfg.BuscaPorCampo('CFG_USU', AUsuarioId);
			if (LCfg.Codigo = 0) then
			begin
		...
		on E: Exception do
		begin
			TLogger.Error('Configuracao.Service.Buscar', E);
			raise; // re-raise to be caught by controller
		end;
	end;
end;

class function TConfiguracaoService.Salvar(const AUsuarioId: Integer; const ABody: TJSONObject): TConfiguracaoResult;
var
	LCfg   : TModelConfiguracaoUsuario;
	LValor : string;
	LNum   : Integer;
...
begin
  ...
  try
    ...
    if LCfg.Codigo = 0 then    
		...
		TLogger.Info('Configuracao.Service: configurações do usuário %d salvas', [AUsuarioId]);
	finally
		LCfg.Free;
	end;
end;
